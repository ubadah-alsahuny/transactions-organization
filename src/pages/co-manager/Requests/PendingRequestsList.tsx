...
import type { CoManagerPendingRequestItem } from '../../../types/request.types';
...
export default function PendingRequestsList() {
  ...
  const [items, setItems] = useState<CoManagerPendingRequestItem[]>([]);

  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const response = await requestsService.listCoManagerPendingRequests({
  ...
  const columns: Array<DataTableColumn<CoManagerPendingRequestItem>> = useMemo(
    () => [
      ...
      { header: 'تاريخ الإنشاء', render: row => formatDateTime(row.created_at) },
      {
        ...
        render: row => (
          <button
            type="button"
            onClick={() => navigate(`/dashboard/co-manager/requests/pending/${row.id}`)}
