...
export default function RequestHistoryList() {
  ...
  const [status, setStatus] = useState<HistoryStatusFilter>('completed');
  ...
  const fetchHistory = async () => {
    setIsLoading(true);
    try {
      const response = await requestHistoryService.listManagerHistory({
        status,
    ...
    () => [
      ...
      { header: 'آخر تحديث', render: row => formatDateTime(row.updatedAt) },
      {
        ...
        render: row => (
          <button
            type="button"
            onClick={() => navigate(`/dashboard/manager/requests/history/${row.requestId}`)}
