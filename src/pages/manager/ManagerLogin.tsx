...
// Try to import logo, fallback will handle if missing
...
export default function ManagerLogin() {
  ...
  const onSubmit = async (data: AuthFormData) => {
    setIsLoading(true);
    try {
      const response = await authService.managerLogin(data.email, data.password);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--color-primary)] px-4">
      <div className="w-full max-w-md rounded-3xl border border-[var(--color-outine)] bg-[var(--color-section)] p-8 shadow-[rgba(0,0,0,0.2)_0_1rem_2rem] backdrop-blur-xl backdrop-saturate-180">
        ...
        <h2 className="mb-6 text-center text-2xl font-bold text-[var(--color-text)]">تسجيل دخول المدير</h2>
        ...
        <div className="mt-6 flex flex-col gap-2 text-center text-sm text-[var(--color-sub-text)]">
          <Link to="/login/co-manager" className="font-semibold text-[var(--color-action)] hover:underline">تسجيل الدخول كنائب مدير</Link>
